  ...
  return (
    ...
    >
      ...
      <div className="flex-grow my-6 space-y-6">
        ...
        {toolsForStep && (
          <div className="p-4 bg-black/20 border border-theme-border rounded-md">
              <h4 className="text-md font-semibold text-theme-accent-secondary mb-3 uppercase tracking-wider">{toolsForStep.title}:</h4>
              <div className="flex flex-wrap gap-2">
                  {toolsForStep.tools.map((tool) => {
                      let colors = 'bg-gray-500/10 text-gray-300 border-gray-500/20 hover:bg-gray-500/20';
                      if (tool.type === 'eq') colors = 'bg-fuchsia-500/10 text-fuchsia-300 border-fuchsia-500/20 hover:bg-fuchsia-500/20';
                      if (tool.type === 'compression') colors = 'bg-sky-500/10 text-sky-300 border-sky-500/20 hover:bg-sky-500/20';
                      if (tool.type === 'reverb') colors = 'bg-indigo-500/10 text-indigo-300 border-indigo-500/20 hover:bg-indigo-500/20';
                      if (tool.type === 'saturation') colors = 'bg-amber-500/10 text-amber-300 border-amber-500/20 hover:bg-amber-500/20';

                      return (
                          <button
                              key={tool.name}
                              onClick={() => handleToolClick(tool.type)}
                              className={`py-1 px-3 rounded-full text-xs font-semibold border transition-colors ${colors}`}
                          >
                              {tool.name}
                          </button>
                      );
